...
!developmentChains.includes(network.name)
    ...
    : describe("Raffle", function () {
          ...
          describe("checkUpkeep", function () {
              ...
              it("returns true if enough time has passed, has players, has enough ETH and is open", async function () {
                  await raffle.enterRaffle({ value: raffleEntranceFee })
                  await network.provider.send("evm_increaseTime", [interval.toNumber() + 1])
                  await network.provider.request({ method: "evm_mine", params: [] })
                  const { upkeepNeeded } = await raffle.callStatic.checkUpkeep([])
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets lottery and sends money to the winner", async function () {
                  const additionalEntrants = 3 //increasing number of people in raffle
                  ...
                  //wait for fulfill random words to be called (no need to wait for local hardhat chain)
                  //need to create a listener to "wait" for the random words to be fulfilled

                  await new Promise(async (resolve, reject) => {
                      //once this event gets emitted
                      raffle.once("WinnerPicked", async () => {
                          //this block gets the event from below
                          try {
                              //so once winner is 'picked', need to do asserts
